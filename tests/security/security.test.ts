...
describe('Security Test Suite', () => {
  ...
  describe('Authentication Security', () => {
    ...
    it('should verify passwords correctly', async () => {
      ...
      const isInvalid = await AuthSecurity.verifyPassword('wrongpassword', hash);
    ...
    it('should generate secure JWT tokens', () => {
      const tokens = AuthSecurity.generateTokens('user123', 'test@example.com', 'user');
  ...
  describe('Access Control', () => {
    it('should enforce role-based permissions', () => {
      expect(AccessControl.hasPermission('user', 'read:own_profile')).toBe(true);
      ...
      expect(AccessControl.hasPermission('admin', 'manage:system_settings')).toBe(false);
      expect(AccessControl.hasPermission('super_admin', 'manage:system_settings')).toBe(true);
    });

    it('should validate resource access', () => {
      expect(AccessControl.canAccessResource('user', 'own_profile', 'read')).toBe(true);
      expect(AccessControl.canAccessResource('user', 'all_users', 'read')).toBe(false);
      expect(AccessControl.canAccessResource('admin', 'all_users', 'read')).toBe(true);
    });
  });
  ...
  describe('Data Encryption', () => {
    ...
    it('should mask sensitive data correctly', () => {
      expect(DataEncryption.maskData('test@example.com', 'email')).toBe('te***@example.com');
  ...
  describe('PII Protection', () => {
    ...
    it('should detect PII in data', () => {
      ...
      expect(PIIProtection.containsPII({ id: '123', publicInfo: 'public' })).toBe(false);
    });
  });
