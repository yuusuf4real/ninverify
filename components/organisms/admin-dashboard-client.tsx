...
export function AdminDashboardClient() {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const revenueResponse = await fetch(
        "/api/admin/transactions?limit=7&sort=created_at&order=desc",
      );
      const revenueData = await revenueResponse.json();

      // Fetch real user growth data
      const usersResponse = await fetch(
        "/api/admin/users?limit=7&sort=created_at&order=desc",
      );
      ...
      if (revenueData.transactions) {
        revenueData.transactions.forEach(
          (tx: {
            id: string;
            status: string;
            type: string;
            ...
            if (tx.status === "completed" && tx.type === "credit") {
              ...
              if (revenueByDay.has(dateStr)) {
                revenueByDay.set(
                  dateStr,
                  revenueByDay.get(dateStr)! + tx.amount,
                );
              }
            }
          },
          ...
          .forEach(
            (tx: {
              ...
              recentActivity.push({
                id: `tx_${tx.id}`,
                type:
                  tx.type === "credit" ? "payment_completed" : "payment_failed",
      ...
      if (usersData.users) {
        usersData.users
          .slice(0, 2)
          .forEach((user: { id: string; email: string; createdAt: string }) => {
            recentActivity.push({
              id: `user_${user.id}`,
      ...
      const processedData: DashboardData = {
        revenueChart: Array.from(revenueByDay.entries()).map(
          ([date, amount]) => ({
            date,
            amount,
          }),
  ...
  const totalRevenue = dashboardData.revenueChart.reduce(
  ...
  const avgDailyUsers =
    dashboardData.userGrowth.length > 0
      ? Math.round(
          dashboardData.userGrowth.reduce((sum, item) => sum + item.count, 0) /
            dashboardData.userGrowth.length,
        )
      : 0;
            ...
            <div className="space-y-4">
              {dashboardData.revenueChart.map((item, index) => {
                const maxAmount = Math.max(
                  ...dashboardData.revenueChart.map((i) => i.amount),
                );
                const width =
            ...
            <div className="space-y-3">
              {dashboardData.userGrowth.map((item, index) => {
                const maxCount = Math.max(
                  ...dashboardData.userGrowth.map((i) => i.count),
                );
