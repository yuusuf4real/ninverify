  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/admin/verifications/analytics?period=${period}`,
      );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Detailed insights into NIN verification performance
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            width: `${Math.max((day.successful / Math.max(...analytics.trends.daily.map((d) => d.total))) * 100, 5)}%`,
                          }}
                        />
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            width: `${Math.max((day.failed / Math.max(...analytics.trends.daily.map((d) => d.total))) * 100, 2)}%`,
                          }}
                        />
        ...
        <Card className="border-border/50">
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Most common verification purposes
            </p>
          </CardHeader>
          <CardContent>
            {analytics.breakdown.byPurpose.length > 0 ? (
              <div className="space-y-4">
                {analytics.breakdown.byPurpose
                  .slice(0, 6)
                  .map((item, index) => (
                    <div
                      key={index}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${(item.count / Math.max(...analytics.breakdown.byPurpose.map((p) => p.count))) * 100}%`,
                            }}
                          />
                        </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium">
                          {formatPurpose(verification.purpose)}
                        </p>
