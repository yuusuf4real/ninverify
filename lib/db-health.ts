...
export async function withDatabaseHealth<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt > 0) {
        console.log(`Database operation retry ${attempt}/${maxRetries}`);
      ...
      if (
        ...
        (error as any)?.message?.includes('Connect Timeout')
      ) {
        if (attempt < maxRetries) {
          console.log(`Database connection failed, retrying in ${(attempt + 1) * 2} seconds...`);
