  ...
  .refine(
    (password) => {
      ...
      const checks = [
        ...
        PASSWORD_COMPLEXITY.requireSpecialChars
          ? /[!@#$%^&*(),.?":{}|<>]/.test(password)
          : true,
...
export const createSecureRequestSchema = <T extends z.ZodRawShape>(
  shape: T,
) => {
  return z
    .object({
...
export function validateApiRequest(req: {
  ...
  if (
    ...
    parseInt(contentLength) >
      parseInt(SECURITY_CONFIG.VALIDATION.MAX_REQUEST_SIZE.replace("mb", "")) *
        1024 *
        1024
  ) {
