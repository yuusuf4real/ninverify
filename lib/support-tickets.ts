...
export function detectIssueType(
  ...
  if (category === "payment_issue") {
    ...
    if (
      paymentKeywords.notInWallet.some((keyword) => lowerDesc.includes(keyword))
    ) {
      subcategory = "money_deducted_not_in_wallet";
      priority = "high";

      // Check for recent successful payments
      const recentPayment = userContext.recentTransactions.find(
        (tx) =>
          tx.status === "completed" &&
    ...
    if (
      paymentKeywords.failedButCharged.some((keyword) =>
    ...
    if (
      paymentKeywords.refundNotReceived.some((keyword) =>
        lowerDesc.includes(keyword),
      )
    ) {
  ...
  if (category === "verification_problem") {
    ...
    if (
      verificationKeywords.ninNotFound.some((keyword) =>
        lowerDesc.includes(keyword),
      )
    ) {
    ...
    if (
      verificationKeywords.documentIssues.some((keyword) =>
        lowerDesc.includes(keyword),
      )
    ) {
...
export function generateTicketSubject(
  ...
  const subjects = {
    payment_issue: {
      ...
      verification_failed_but_charged:
        "Verification failed but payment was charged",
  ...
  return (
    categorySubjects?.[subcategory] ||
    categorySubjects?.default ||
    "Support request"
  );
}
...
export function generateSystemMessage(
  ...
  const responseTime = formatDuration(
    Date.now(),
    slaDeadlines.firstResponseDue.getTime(),
  );
